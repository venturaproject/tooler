...
open connection to block on. Four tools give an MCP-driven agent the same "one task at
...
- **`tooler_play_repl_list`** (no arguments) lists every open session still on disk — `session_id`, `playbook_name`, and `task_count` for each — for recovering a `session_id` lost from context, or auditing for sessions nobody ever `_close`d. Read-only; it never deletes anything, including a session whose files turn out to be corrupt (listed with an empty name and `task_count: 0` rather than skipped or erroring the whole call).
...
eventually by the OS's own temp-directory policy like any other stray file — `_list`
above helps you find and manually `_close` one, but there's still no active reaping.
...
**`--audit-log <path>`** (also settable via `TOOLER_PLAY_AUDIT_LOG`) appends one JSON line per task attempt — `{ts, playbook, task, action, status, duration_ms, error, error_kind}` — to `path`, the same append-only mechanism [`tooler mcp --audit-log`](#tooler-mcp) already uses for MCP tool calls. It covers every concrete attempt: top-level tasks, tasks nested in `block:`/`rescue:`/`always:`/`parallel:`/`include:`, and each `loop:` iteration separately — but not a task skipped via `when:`, since that never touched anything. A write failure (e.g. an unwritable path) is reported to stderr and never fails the task itself. This is the persistent execution trail terminal scrollback alone can't give you once a playbook is restarting services, killing processes, or writing files unattended via `tooler cron local`.
...
**`--list-tasks`**/**`--list-tags`** inspect a playbook's shape with zero side effects: just parse the YAML and print, without resolving `vars_files:`/secrets or touching a server/database/connection of any kind — useful for an agent (or a human) deciding what to run before committing to `--start-at-task`/`--tags`/`--skip-tags`, without having to parse the task DSL itself. `--list-tasks` prints every task's name, action, and tags, including tasks nested in `block:`/`rescue:`/`always:`/`parallel:` (an `include:` task shows its target filename but isn't recursed into — that's a separate file); `--list-tags` prints every distinct tag used anywhere in the playbook, sorted and deduplicated. Both respect `--tags`/`--skip-tags` on the listing itself, so it matches what a real run would actually attempt; both support `--output json` for a structured `{playbook, tasks: [...]}` / `{tags: [...]}` shape. Any task that's one of the `confirm:`-gated destructive actions (`fs_write:`/`systemd_restart:`/`ps_kill:`/`db_exec:`/`db_load:`/`secret_set:`/`deploy:`/`upload:`, and `cron:`/`template:` when they mutate remote state — see Trust model below) also carries a `confirmed` field in `--list-tasks`'s JSON — `true`/`false` for whether its own YAML already sets `confirm: true`, entirely absent for every non-destructive task — so an agent about to run a playbook with `yes: true` can see its whole blast radius (`tasks[] | select(.confirmed == false)` in `jq`) without reading the YAML by hand. The plain-text listing shows the same thing as a `[needs confirm:]` marker, but only on the risky case — an already-confirmed or non-destructive task prints nothing extra, same "silence means safe" convention `--lint`'s findings already use.

**`--lint`** is the same zero-side-effect parsing turned into static analysis — it never resolves `--tags`/`--skip-tags`, never touches a server, and always exits `0`; findings are advisory, printed as `{playbook, findings: [...]}` under `--output json`. Four checks: a `register:`ed `http:`/`scrape:`/`db_query:`/`mail_check:` result (the same untrusted-source examples the Trust model section below names) reaching `run:`/`ssh:`/`fleet:` without a `| quote` filter — the shell-injection risk `| quote` exists to close, surfaced before a real run instead of only after one; a `{{var}}` reference (in `run:`/`ssh:`/`fleet:`'s command, or `debug:`/`assert:`/`when:`/`changed_when:`/`failed_when:`/`until:`) that no earlier `vars:`/`vars_files:`/`register:`/`set_fact:` defines — a likely typo; and — reading only already-loaded local state (`ctx.config`, the OS keychain's existence probe, never a network connection) — a literal `server:` field naming a profile that was never `tooler server add`ed (or `tooler config set mail.<name>...` for `mail:`/`mail_check:`'s separate mail-profile namespace), and a `{{secret.<profile>.<key>}}` token whose value isn't actually set. Neither `--dry` nor the rest of `--lint` caught this before — every remote-action dispatch only resolves its `server:` *inside* its `if !env.dry` branch, so a typo'd server name was previously only ever discovered on a real run. It's a heuristic, not a formal verifier: it doesn't trace taint through a dynamic `loop: {from: ...}` into `{{item}}`, it stops checking for undefined vars after an `include_vars:` task (its target's contents aren't known statically), it can't see a var only ever supplied via `--var`/`--vars-file` at invocation time — that one's expected to be flagged, not a bug — and a templated `server:`/`{{secret.*}}` value (containing `{{`) can't be statically resolved either, so it's silently skipped rather than guessed at. `fleet:`'s `servers:` (each comma-separated name checked individually against `ctx.config.server`, same as `ssh:`) and `group:` (checked against `ctx.config.group`) are covered too; `fleet: {all: true}` needs no check at all — targeting every configured server is always resolvable. The fourth check catches things that only fail (or silently mis-resolve) at run time: a `notify:` that names no `handlers:` entry (a runtime hard error, surfaced here before the run), and an `include:` / `include_vars:` / `vars_files:` path — when it's a literal, not a `{{templated}}` one — that resolves to no file on disk (a missing `vars_files:` was previously swallowed silently).

**`--explain`** sits between `--dry` (shape) and `--lint` (problems): it resolves every task's `{{vars}}`/`{{state.*}}` and prints the **concrete action each one would take** — the exact command line, the rendered SQL, the target server and file path — as `{playbook, tasks: [{name, action, explain: {...}}]}` JSON, without running anything or opening a single connection (always exits `0`). `set_fact:`/`state_set:` values are threaded forward so later tasks resolve against them; a `register:`ed value can't be known statically, so it stays as the literal `{{name}}` — the honest answer. This is an agent's pre-flight: "show me exactly what will happen before I say go." Available over MCP as `tooler_play` with `{"explain": true}` (and with `content:`, like the other inspection modes).
...
The `tooler_play` MCP tool also accepts `content: "<yaml>"` instead of `file:` — inline playbook text, written to a short-lived temp file for that one call and deleted immediately after, so an agent inspecting a playbook it just drafted doesn't need a separate write-to-disk round trip first. Mutually exclusive with `file:`, and only valid combined with `dry: true`, `lint: true`, `list_tasks: true`, `list_tags: true`, or `explain: true` — a *real* run still needs a real file. This is deliberate, not an oversight: the whole point of `confirm:`/`Confirmed<T>` (see Trust model below) is that a destructive action only ever runs from YAML a human reviewed, and `content:` letting an agent skip straight from "text I just generated" to a real run would cut against that — so `content:` unlocks exactly the inspection workflows this section already covers (`--list-tasks`/`--list-tags`/`--lint`/`--dry`/`--explain`), and nothing more.

**Playbooks as MCP tools.** A playbook that declares an `mcp_tool:` block (with a `description:`, and optional `params:` giving each input a `type`/`description`/`required`/`enum`/`default`) is exposed by `tooler mcp` as its own first-class tool, `tooler_pb_<name>`, alongside the generic `tooler_play` — so an agent calls a proven process by name with typed, documented arguments instead of hand-assembling YAML. When the playbook contains any `confirm:`-gated action the tool's schema also offers a `confirm` boolean; passing `confirm: true` runs it with `--yes`. `params:` also seed `vars:`, so a playbook with a `params:` block still runs from the plain CLI (`tooler play deploy --var env=prod`). Discovery is fixed at server startup from `./playbooks/` relative to the launch directory — run `tooler mcp` from the project root; `tooler mcp --list-playbook-tools` prints what would be exposed without starting the server.

```yaml
name: Deploy web
mcp_tool: {description: "Deploy the web app to an environment"}
params:
  env: {type: string, required: true, enum: [prod, staging]}
  skip_tests: {type: boolean, default: false}
tasks:
  - name: deploy
    deploy: {server: "web-{{env}}", path: /srv/app, pull: true, restart: "systemctl restart app", confirm: true}
```
...
| `http: {method, url, headers, body, timeout, ignore_status, download, paginate}` | Make an HTTP request; `download:` saves the (binary-safe) body to a local file; `paginate:` follows every page into one array |
...
| `parallel: [...]` | Run a list of tasks concurrently (cap with `max_parallel:`), joining before the next task |
...
| `template: {src, dest, server, confirm}` | Render a Jinja-style template file (`{% for %}`/`{% if %}`) to a local path, or push it to a server (`server:` requires `confirm: true`) |
...
| `db_load: {server, table, file, format, mode, columns, headers, env/engine/host/…, confirm}` | Bulk-load a local CSV/TSV/JSON file into a remote table (`\copy` / `LOAD DATA`) — requires `confirm: true` |
| `deploy: {server, path, pull, build, restart, health_url, health_timeout, health_retries, health_delay, sudo, sudo_pass, confirm}` | Pull, build, restart, and health-check a remote app — requires `confirm: true` |
| `upload: {server, local, remote, confirm}` | Push a local file to an absolute remote path over scp — requires `confirm: true` |
| `cron: {server, add / remove / list, confirm}` | Manage a remote crontab (exactly one of add/remove/list); `add`/`remove` require `confirm: true` |
...
`parallel:` runs its child tasks **concurrently** — each on its own thread with a cloned copy of the vars (the same `std::thread::scope` fan-out `max_parallel:` `loop:` uses) — joining before the playbook moves on. `max_parallel: N` on the same task caps how many run at once (default: all). It's for independent steps: extract from three sources at once, run per-region loads in parallel. After the children join, every var each one added or changed (a `register:`, a `set_fact:`) is merged back in child order — deterministic despite the concurrency; the first child that fails in that order fails the `parallel:` task. Counts as one recap outcome, like `block:`. Can't be combined with `loop:` or `block:`/`rescue:`/`always:` on the same task, and a `confirm:` (interactive pause) child is rejected — an agent-run playbook uses `--yes` anyway.
...
`template:` is `write_file:` with a real template engine ([MiniJinja](https://github.com/mitsuhiko/minijinja) — Jinja2-compatible: `{% for %}`, `{% if %}`, `{{ x.field }}`, filters) — for a config that isn't just variable substitution: an `nginx.conf` with one `upstream` block per backend, a `systemd` unit, a per-environment `.env`. `src:` is a local template file (confined to the playbook directory); every playbook var is exposed to the template as parsed JSON where it parses (so `{% for row in rows %}` iterates a `register:`ed array) and as a plain string otherwise, plus `now`. It writes `dest:` locally (confined, like `write_file:`) unless `server:` is set — then it pushes the rendered file to that remote path over SSH the same way `fs_write:` does, and requires `confirm: true`. `register:` captures the byte count; `--diff` previews the change. This is the "generate a config file without shelling out to `python -m jinja`" primitive.
...
`http:`'s `paginate:` follows a paginated API into a single `register:`ed array — the loop an agent otherwise hand-rolls from `loop:`+`until:`+`set_fact:`. After each page, `next:` is rendered with that page's body available as `{{page}}` (and `{{page.status}}`); its result is the next URL, and the loop stops when it renders blank, unchanged, or `max_pages:` (default 20) is hit. `items:` (a `| json:`-style path) points at the array inside each page — every page's array is concatenated; omit it to collect whole page bodies instead. `register:` then also gets `<reg>.pages` (the page count) alongside `<reg>.status`. Mutually exclusive with `download:`.

```yaml
- name: pull every order since the cursor
  http:
    url: "https://api.shop.com/orders?after={{state.cursor | default:}}"
    paginate: {next: "{{page | json:links.next}}", items: "data"}
  register: orders
```
...
`db_load:` is the bulk-load counterpart — the ETL "L" that `db_exec:` (one row per SSH round trip) and `sync_db:` (a whole-database dump/restore) don't cover. It streams a local `file:` into a remote `table:` over **one** SSH connection: Postgres via `psql -c "\copy … FROM STDIN"`, MySQL via `mysql --local-infile=1 -e "LOAD DATA LOCAL INFILE '/dev/stdin' …"`. `file:` is confined to the playbook directory (typically a `write_csv:`/`write_file:` output from an earlier task); `format:` is `csv` (default), `tsv`, or `json` (a JSON array, converted to CSV in-process first); `mode:` is `append` (default), `truncate` (empties the table first), or `upsert` (MySQL only — `LOAD DATA REPLACE`; Postgres upsert needs a staging table by hand). `columns:` sets an explicit target column list; `headers:` (default true) skips the file's first line. Same connection fields and `confirm: true` gate as `db_exec:`. `register:` captures the client's summary line (`COPY 42` on Postgres). Table/column names are validated (`[A-Za-z_][A-Za-z0-9_.]*`) since they can't be parameterized — never build them from untrusted data.

`deploy:` is the native playbook equivalent of [`tooler deploy run`](#tooler-deploy) — pull the latest code (`pull: true`), run a build step (`build: "<command>"`), restart a service (`restart: "<command>"`, `sudo: true`/`sudo_pass:` if it needs it), and health-check the result (`health_url:`, with `health_timeout:`/`health_retries:`/`health_delay:` tuning the retry loop — 5s/3 attempts/2s between by default, same as the CLI command's own defaults), each step optional. Both the standalone command and this task run through the exact same underlying function, so the error wording ("git pull failed: ...", "build failed: ...", "restart failed: ...", "health check failed after N attempt(s): ...") is identical either way. Requires `confirm: true`, same non-negotiable gate `db_exec:`/`fs_write:`/etc. use — pulling code, running an arbitrary build command, and restarting a service are at least as destructive as any of those. `timeout:` isn't supported on `deploy:` yet (unlike `run:`/`ssh:`/`fleet:`/`sync_db:`/`sync_files:`).

`upload:` pushes a single local file to an absolute remote path over `scp` — the same primitive [`tooler ssh copy`](#tooler-ssh) uses, so the connection-failure wording matches. `local:` resolves relative to the playbook's own directory (an absolute path is taken as-is), and is an author-time path — unlike `write_file:`'s `path:` it isn't confined, since pushing a build artifact from `target/` or `dist/` is the normal case; `write_file:` covers "write rendered content somewhere predictable," `upload:` covers "put this exact file (a binary, a tarball, a cert) on the server." Requires `confirm: true`. `register:` (if set) captures the local file's byte count.

`cron:` manages a remote server's crontab — the same engine as [`tooler cron`](#tooler-cron). Exactly one of `add: "<full crontab line>"`, `remove: "<fixed substring>"` (drops every matching line), or `list: true`. `add`/`remove` mutate the crontab and require `confirm: true`; `list` is read-only and needs none. `register:` captures `"true"` for `add`, the count of removed lines for `remove`, and a JSON array of `{line, schedule, command}` entries for `list` — directly `loop: {from: "{{reg}}"}`-chainable, e.g. to assert a scheduled job is installed.

`on_failure:` is a playbook-level list of tasks (a sibling of `tasks:`/`handlers:`) that runs when the task loop hits a non-ignored failure, right before the run reports and bails — the "if anything fails, alert someone / record it" hook that `block:`'s `rescue:` only provides at block scope. It's best-effort: a failing `on_failure:` task is logged and the rest still run, it never re-triggers itself, and the run still exits non-zero on the original failure. Skipped entirely in `--dry`, and not fired by a handler failure. Two vars are set for its tasks: `{{failed_task}}` (the name of the task that failed) and `{{failure_reason}}` (its error text) — enough to send a useful alert:
...
on_failure:
  - name: alert ops
    mail:
      server: notify
      to: "ops@example.com"
      subject: "playbook failed at {{failed_task}}"
      body: "{{failure_reason}}"
  - name: remember the failure for the next scheduled run
    state_set:
      last_failure: "{{failed_task}}"
```

**`single_instance: true`** (a top-level playbook field, alongside `tasks:`) takes an exclusive `<file>.lock` for the duration of a real run. A second `tooler play` of the same file while the lock is held exits non-zero with a clear message (naming the holding pid, host, and start time) instead of racing the first run's `state_set:` writes — the guard a `tooler cron local`-scheduled playbook needs when a run occasionally overruns its interval. A lock older than `lock_timeout:` seconds (default 21600 = 6h) is assumed abandoned (the previous run was killed) and taken over with a warning. The lock is released on any exit — success, failure, or panic — and a stale one is cleaned by the next run either way, so a hard kill self-heals. Ignored in `--dry` and for `include:`d sub-playbooks (only the outermost run locks). Gitignore `*.lock` alongside `*.state.json`/`*.data.json`.
...
- `timeout: N` — kill the task if it's still running after N seconds. Supported on `run:` (kills the local subprocess), `ssh:`/`fleet:` (kills the `ssh` process, ending the remote command along with it), and `sync_files:`/`sync_db:` (same `ssh` process kill — an `rsync` or a `pg_dump`/`mysqldump`→`psql`/`mysql` pipe can hang just as easily on a flaky connection or a huge transfer). `sync_db:` runs two separate `ssh` invocations (dump, then restore) — `timeout:` applies to each independently, so a `timeout: 60` sync can take up to ~120s worst case, not a single 60s budget for the whole task. Every other action rejects `timeout:` upfront rather than silently not honoring it.
...
**Templating** — `{{...}}` inside any string field resolves, in order: a playbook/`--var` variable, then `env.<NAME>` (the process environment, e.g. `{{env.HOME}}`), then `secret.<profile>.<key>` (the OS keychain, the same store `tooler config set profile.<name>.token` and OAuth2 profiles already use — e.g. `{{secret.exact.token}}`). `secret_set: {profile, key, value, confirm}` is the write-side counterpart — stores `value` under that same `profile`/`key` so a later `{{secret.*}}` (in this run or any future one) reads it back; `value:` renders normally, so `value: "{{secret.old.key}}"` copies/rotates a secret between profiles with no special-casing. Requires `confirm: true`, same gate `db_exec:`/`fs_write:` use — never written to disk, never printed. Anything that doesn't resolve is left exactly as written, so a missing var/secret never crashes a playbook, it just doesn't get substituted. `{{token | json:path.to.field}}` applies a filter after resolving `token`: parses its value as JSON and walks a dot-separated path (`data.id`, `items[0].name`, `[2]`) into it — a string leaf renders raw, anything else (number/bool/object/array/null) renders as JSON text. A path ending in `length` (`{{prs | json:length}}`, `{{resp | json:data.items.length}}`) returns the element/key/char count of an array/object/string instead of doing a field lookup — pairs naturally with `when:`'s numeric operators, e.g. `when: "{{prs | json:length}} > 0"`. Invalid JSON or a path that doesn't match leaves the whole `{{...}}` literal, same as any other unresolved token — it never fails the render. `{{token | quote}}` POSIX single-quote-escapes `token`'s value for safe interpolation into a `run:`/`ssh:`/`fleet:` shell command line (see the **Trust model** note below) — always succeeds, unlike `json:...`. `{{now}}` is always available — the current instant as RFC3339 UTC (a playbook var named `now` still wins). A `'quoted'` token is a **string literal**, so a pipeline can start from a constant without a throwaway var: `{{ '[1,2,3]' | sum }}`, `{{ 'payload' | sha256 }}`.

**Filters chain**, left to right: `{{rows | where:active==true | pluck:name | join:, }}`. Every filter except `default:` is skipped when the value is unresolved, and any shape mismatch (bad JSON, wrong type, unparseable date) leaves the whole `{{...}}` literal — the same fail-soft rule a bad `json:` path already has, so a filter never crashes a render. The full set:

- **structure** — `json:<path>` (walk into JSON, `length` at the end for a count), `default:<value>` (the one filter that produces something from an *unresolved* token — `{{state.cursor | default:0}}` on a first run), `pluck:<field>` (array of objects → array of that field), `where:<field>==<value>` / `where:<field>!=<value>` (filter an array of objects), `sort` / `sort_by:<field>` / `unique` / `reverse` / `slice:<a>:<b>` (Python-ish, negative indices), `join:<sep>`, `first` / `last`.
- **numbers** — `add:<n>` / `sub:<n>` / `mul:<n>` / `div:<n>`, `round` / `round:<places>`, and the aggregates `sum` / `min` / `max` / `avg` / `count` (over a JSON array).
- **dates** — `date:<strftime>` (reformat), `shift:<±N[smhdw]>` (`{{now | shift:-7d | date:%Y-%m-%d}}` = 7 days ago), `unix` (epoch seconds). The value parses as RFC3339, a unix integer, or `%Y-%m-%d[ %H:%M:%S]`.
- **encoding / hash** — `base64` / `base64d`, `urlencode`, `sha256` / `sha1` / `md5` (hex digest), `hmac_sha256:<keyref>` (hex HMAC; `<keyref>` is a var name, typically set from `{{secret.*}}` via `set_fact:`).
- **strings** — `upper` / `lower` / `trim`, `quote` (POSIX-escape for a shell command line — see **Trust model** below).

These cover the data-wrangling, date math, and checksums a playbook would otherwise shell out to `jq`/`date`/`openssl` for. **Security note**: a rendered secret ends up in a `run:` task's shell command line, which — like any subprocess argv — is visible to other local processes via `ps`/`/proc` while it runs; `ssh:`/`fleet:` carry the same exposure over SSH, no different from how `sudo:` already works today. What gets **printed** to the console for `run:`/`ssh:`/`fleet:`/`sync_files:` is separately masked — a `{{secret.*}}` token always shows as `***` in the echoed command line, even though the real, unmasked value is what actually runs; `debug:` is the one exception, since printing is its entire purpose.
...
**Trust model** — `run:`/`ssh:`/`fleet:` render `{{var}}` straight into a shell command line with no escaping, by design: that's what makes `run:` a general-purpose "run a shell command" primitive rather than a fixed-argument one, the same tradeoff Ansible's own `shell:` module makes. That's fine when a var comes from `--var`/`vars:`/`{{secret.*}}` (values *you* control), but if a var's value instead came from `http:`/`scrape:`/`db_query:` against **untrusted** data — a third-party API response, a scraped page, rows an attacker could influence — treat feeding it straight into a later `run:`/`ssh:` task the same way you'd treat `eval`-ing untrusted input in any other language: pipe it through the `| quote` render filter (see **Templating** above) before it reaches a `run:`/`ssh:`/`fleet:` command line, and validate its shape first with `assert:` — or avoid piping it into a shell task at all. `write_file:`'s `path:` is confined to the playbook's own directory (an absolute path or a `..` that nets outside it is rejected) precisely because *its* one job is "write somewhere predictable" — `run:`/`ssh:` make no such promise, since restricting them would defeat their purpose. `db_exec:`, `db_load:`, `fs_write:`, `ps_kill:`, `systemd_restart:`, `secret_set:`, `deploy:`, `upload:`, `cron:` (when adding or removing a crontab line), and `template:` (when `server:` pushes it to a remote path) — the task actions that mutate, restart, or terminate state outside the playbook itself (a database row, a bulk table load, a remote file, a remote service, a remote process, a keychain entry, a deployed application, a file pushed to a server, a scheduled job, a rendered config on a server) — all require `confirm: true` written literally in the YAML before they'll run at all, checked before any of them ever resolves a server or opens a connection: a deliberate write always looks deliberate in the source, never just "whatever the last run happened to do." Finally, `tooler play --audit-log <path>` (also `TOOLER_PLAY_AUDIT_LOG`) appends one JSON line per task attempt — timestamp, playbook, task, action, status, duration, error, error_kind — to a file of your choosing, the same mechanism [`tooler mcp --audit-log`](#tooler-mcp) already uses for MCP tool calls; a task skipped via `when:` isn't logged, since it never touched anything. It's the persistent record that terminal scrollback alone can't give you once a playbook is restarting services, killing processes, and writing files unattended via `tooler cron local`.

**Secret redaction in error output.** A task's *printed preview* of a command masks `{{secret.*}}` (above), but its real, unmasked argv is what actually runs — and a subprocess (`run:`, or the remote end of `ssh:`/`fleet:`/`http:`) can echo that argv back in its own stderr on failure, independent of anything tooler itself prints. Before a task's error is recorded anywhere — `--output json`'s `error` field or `--audit-log`'s line — every `{{secret.*}}` value that task's own fields reference is resolved for real and scrubbed from it (`***`), so a leak from a subprocess's own output is caught the same as a leak from tooler's own interpolation would be. This only covers the *current* task's own referenced secrets, and only the recorded error text — not the live console output during an interactive run (the same "your own terminal" trust boundary `debug:` already relies on).
