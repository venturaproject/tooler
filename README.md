...
**`--skip-tags <tags>`** is the complement of `--tags`: a comma-separated list of tags to exclude. A task must match `--tags` (if given) *and* not match any `--skip-tags` entry to run — `--tags build --skip-tags build` therefore runs nothing. Same top-level-only scoping `--tags` already has (no effect inside `include:`/`block:`).
